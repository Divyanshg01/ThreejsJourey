...
void main()
{
    ...
    // float strength = step(0.8, mod(vUv.y * 10., 1.));
    ...
    // float barX = step(0.4, mod(vUv.x * 10. - 0.2, 1.));
    // barX *= step(0.8, mod(vUv.y * 10., 1.));
    // float barY = step(0.8, mod(vUv.x * 10., 1.));
    // barY *= step(0.4, mod(vUv.y * 10. - 0.2, 1.));
    // float strength = barX + barY;
    ...
    // strength *= 1.0 - step(0.25, max(abs(vUv.y - 0.5), abs(vUv.x - 0.5)));
    // float strength = floor(vUv.x * 10.0) / 10.;
    // strength *= floor(vUv.y * 10.0) / 10.;
    // float strength = random(vUv);
    // vec2 gridUV = vec2(floor(vUv.x * 10.0) / 10., floor(vUv.y * 10.0) / 10.);
    ...
    //         vUv.y
    //     );
    // float strength = .015 / distance(lightUV, vec2(0.5, 0.5));
    // vec2 lightUVX = vec2(
    //         vUv.x * 0.2 + .4,
    //         vUv.y
    //     );
    // float strengthX = .015 / distance(lightUVX, vec2(0.5, 0.5));
    //
    // vec2 lightUVY = vec2(
    //         vUv.x,
    //         vUv.y * 0.2 + 0.4
    //     );
    //
    // float strengthY = .015 / distance(lightUVY, vec2(0.5, 0.5));
    // float strength = strengthY * strengthX;
    // vec2 rUv = rotate(vUv, 1.0, vec2(0.5, 0.5));
    // vec2 lightUVX = vec2(
    //         rUv.x * 0.2 + .4,
    //         rUv.y
    //     );
    ...
    // float strength = 1. - step(0.01, abs(distance(vUv, vec2(0.5, 0.5)) - 0.25));
    // vec2 waveuv = vec2(
    //         vUv.x,
    //         vUv.y + sin(vUv.x * 30.) * 0.1
    //     );
    // float strength = 1. - step(0.01, abs(distance(waveuv, vec2(0.5, 0.5)) - 0.25));
    // vec2 waveuv = vec2(
    ...
    // float strength = angle;
    // float angle = ((atan(vUv.x - 0.5, vUv.y - 0.5) / pi) + 1.) / 2.;
    // float strength = angle;
    // float angle = ((atan(vUv.x - 0.5, vUv.y - 0.5) / pi) + 1.) / 2.;
    ...
    // float angle = ((atan(vUv.x - 0.5, vUv.y - 0.5) / pi) + 1.) / 2.;
    // float s = sin(angle * 100.);
    // float radius = 0.25 + s * 0.02;
    // float strength = 1. - step(0.01, abs(distance(vUv, vec2(0.5, 0.5)) - radius));

    // float strength = cnoise(vUv * 10.);
    // float strength = 1.0 - abs(cnoise(vUv * 10.));
    // float strength = sin(cnoise(vUv * 10.) * 20.);
    float strength = step(0.7, sin(cnoise(vUv * 10.) * 20.));
    vec3 col = vec3(vUv, 1.0);

    // if unwanted extrapolation of strength which causes whiteness then clamp the strength
    // clamp(strength , 0. ,1.)
